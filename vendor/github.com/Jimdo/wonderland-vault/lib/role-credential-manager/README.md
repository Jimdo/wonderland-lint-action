...
func main() {
    ...
    r, err := rcm.New(
        ...
        rcm.AddAWSClientConfigs(&elb.Config),
    )
    if err != nil {
...
If you need to change the configuration afterwards, you can call `Configure(...)` with other options.
...
func main() {
    ...
    if err = r.Configure(rcm.WithLogger(anotherLogger)); err != nil {
        log.Fatalf("Error initializing RCM library: %s", err)
    }
}
```
